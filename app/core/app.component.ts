...
import {RouteConfig, Route, RouterOutlet, RouterLink, Router} from "angular2/router"; //S's original
...
@Component({ //a component controls a portion of the screen — a view — through its associated template.
	selector: "app", //The CSS selector, ie, HTML element that represents this component is "app", which is not a unique name on the tag, but a type selector, or possible a class-based component selector.
...
@RouteConfig([/*assign a router to the component and configure that router with routes. Routes tell the router which views to display when a user clicks a link or pastes a URL into the browser address bar*/
	//{ path: "/", component: HomeComponent, as: "HomeComponent", data: undefined } // the as serves as alias for links, etc. this was how S set up route config.
	...
	//the below are 4 more pages' route configs. mimic Tour of Heroes guidelines. copy, cut and paste them into their proper name.component.ts files
	{
